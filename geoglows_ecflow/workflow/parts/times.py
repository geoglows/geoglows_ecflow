from geoglows_ecflow.workflow.comfies.ooflow import Family, Task, Variable
from geoglows_ecflow.workflow.comfies.ooflow import TimeSlot, TimeSeries
from geoglows_ecflow.workflow.comfies.dateandtime import Date, DAY, Time
from geoglows_ecflow.workflow.comfies.dateandtime import CalSeq, future
import datetime
import ecflow
...
def t2t(t):
    ...
    if isinstance(t, ecflow.Time):
        ...
        raise TypeError(
            'expected ecflow.TimeSlot, ecflow.Time, ecflow.TimeSeries or datetime.time, not {}'.format(type(t)))
...
class TimerDate(object):
    ...
    def __init__(self, monthday, time):
        if isinstance(time, Time):
            time = str(time)
        if isinstance(time, datetime.time):
            time = '{02d}:{02d}'.format(time.hours, time.minutes)
    ...
    def __str__(self):
        return str(self.monthday, self.time)

    def add_to(self, node):
        node.add_time(self.time)
        node.add_date(self.monthday, 0, 0)


class CronDataAvailability(Family):
    ...
    def __init__(
        ...
        super(CronDataAvailability, self).__init__(name)

        t = Task('check_mars')

        self.ymd = Variable('YMD', base.ymd)
        t.add(self.ymd)

        # display some info
        t.add_label('base', base.ymd)
        t.add_label('YMD', base.ymd)

        # task will be executed once a day by attached cron
        cron = ecflow.Cron()
        cron.set_time_series(TimeSeries(
            TimeSlot(0, 0), TimeSlot(23, 00), TimeSlot(23, 0)))
        t.add_cron(cron)
        self.add(t)

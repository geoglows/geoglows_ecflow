...
class RAPIDDataset(object):
        ...
        if 'time' in self.qout_nc.variables.keys():
            if len(self.qout_nc.dimensions['time']) > 0:
                if not is_masked(self.qout_nc.variables['time'][:]):
                    try:
                        timestep = (
                                datetime.datetime.utcfromtimestamp(self.qout_nc.variables['time'][1]) -
                                datetime.datetime.utcfromtimestamp(self.qout_nc.variables['time'][0])
                            # datetime.datetime.fromtimestamp(self.qout_nc.variables['time'][1], datetime.UTC) -
                            # datetime.datetime.fromtimestamp(self.qout_nc.variables['time'][0], datetime.UTC)
                        ).total_seconds()
                        if timestep > 0:
                            time_var_valid = True
                    except ValueError:
    ...
    def write_flows_to_csv(self, path_to_output_file,
        ...
        if self.is_time_variable_valid() or self._is_legacy_time_valid():
            ...
            log("Valid time variable not found. Printing values only ...", "WARNING")
